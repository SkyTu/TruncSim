__all__ = ["Number", "FixedPointWithProbError", "FixedPoint", "BlockFloatingPoint", "FloatingPoint"]
...
class FixedPointWithProbError(Number):
    r"""
    ...
    *Deep Learning with Limited Numerical Precision* (https://arxiv.org/abs/1502.02551)
    ...
    def __init__(self, wl, fl, pp, pv, np, nv, clamp=True, symmetric=False, trunc=True,tensor_type=32):
        assert pp >= 0 or pp <= 1, "invalid probability for positive error: {}".format(pp)
        assert np >= 0 or np <= 1, "invalid probability for negative error: {}".format(np)
        assert type(pp) == float, "invalid type for positive error value: {}".format(pp)
        assert type(np) == float, "invalid type for negative error value: {}".format(np)
        assert type(pv) == int, "invalid type for positive error value: {}".format(pv)
        assert type(nv) == int, "invalid type for negative error value: {}".format(nv)
        ...
        assert tensor_type in (32, 64), "invalid tensor type: {}, only support 32 or 64".format(tensor_type)
        self.wl = wl
        self.fl = fl
        self.pp = pp
        self.pv = pv
    ...
    def __str__(self):
        return "FixedPoint (wl={:d}, fl={:d}, pp = {}, pv = {}, np = {}, nv = {})".format(self.wl, self.fl, self.pp, self.pv, self.np, self.nv)
