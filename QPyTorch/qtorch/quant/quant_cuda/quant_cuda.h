 ...
 * and fractional bits [fl], with option of clamping the over/underflow numbers
 * having a symmeric number range.
 * Probabily Cause Error due to the MPC truncation.
 **/

Tensor fixed_point_quantize_prob_error_cuda(Tensor a, int wl, int bitlength, int fl, int trunc_type, bool clamp, bool trunc, int type);
