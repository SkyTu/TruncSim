...
// 需要加一个判断是否是trunc，初始化的时候是不需要加误差的
// 需要考虑符号翻转的误差概率
template <typename T>
Tensor fixed_point_quantize_prob_error(Tensor a, int wl, int bitlength, int fl, int trunc_type, bool clamp, bool trunc){

  CHECK_INPUT(a);
  auto a_array = a.data_ptr<T>();
  Tensor o = zeros_like(a);
  auto o_array = o.data_ptr<T>();
  ...
  if(trunc){
    for (int64_t i = 0; i < size; i++)
    {
      o_array[i] = ldexp(a_array[i], -sigma);
      //Test overflow
      
      if (trunc_type == 1){
        T pr1 = o_array[i] - std::trunc(o_array[i]) +1 - std::trunc(o_array[i] - std::trunc(o_array[i])+1);
        pr1 =  o_array[i] > 0 ? pr1:1-pr1;
        o_array[i] = o_array[i] + prob_gen(pr1);
      }

      if (trunc_type == 2){
        T pr1 = o_array[i] - std::trunc(o_array[i]) +1 - std::trunc(o_array[i] - std::trunc(o_array[i])+1);
        pr1 =  o_array[i] > 0 ? pr1:1-pr1;
        o_array[i] = o_array[i] > 0 ? o_array[i] + prob_gen(pr1): o_array[i] - prob_gen(pr1);
        T pr2 = o_array[i] > 0 ? o_array[i] / ldexp(1, wl+sigma): -o_array[i] / ldexp(1, wl+sigma);
        int tmp = prob_gen(pr2);
        o_array[i] = o_array[i] > 0 ? o_array[i] - ldexp(1, wl-fl) * tmp: o_array[i] + ldexp(1, wl-fl) * tmp;
      }
      o_array[i] = std::trunc(o_array[i]);
      o_array[i] = ldexp(o_array[i], sigma);
    }  
  }
  return o;
}
...
{
  // m.def("fixed_point_quantize_prob_error_half", &fixed_point_quantize_prob_error<at::Half>, "Fixed Point Number Probabilistic Quantization with Error (CPU)");
  m.def("fixed_point_quantize_prob_error_float", &fixed_point_quantize_prob_error<float>, "Fixed Point Number Probabilistic Quantization with Error (CPU)");
  m.def("fixed_point_quantize_prob_error_double", &fixed_point_quantize_prob_error<double>, "Fixed Point Number Probabilistic Quantization with Error (CPU)");
