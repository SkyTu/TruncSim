...
class LeNet(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(256, 120),  # Assuming the output of the flatten layer is 256
            nn.ReLU(),
            nn.Linear(120, 84),
            nn.ReLU(),
            nn.Linear(84, 10)  # 10 output classes for classification
        )
    def forward(self, x):
...
optimizer = optim.SGD(params=model_lenet.parameters(), momentum =0.90625,lr=0.015625)
...
for epoch in tqdm(range(EPOCHS)):
    ...
    for X, y in train_dataloader:
        ...
        if cnt % 10 == 0:
            ...
            with open(f'output/lenet/plaintext.txt', 'a') as f:
                f.write(f'{accuracy:.2f}\n')
        cnt += 1
with open(f'output/lenet/highest_plaintext.txt', 'a') as f:
