...
class CNN2(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(256, 128),  # Assuming the output of the flatten layer is 256
            nn.ReLU(),
            nn.Linear(128, 10)  # 10 output classes for classification
        )
    def forward(self, x):
...
optimizer = optim.SGD(params=model_cnn2.parameters(), momentum =0.90625,lr=0.015625)
...
for epoch in tqdm(range(EPOCHS)):
    ...
    for X, y in train_dataloader:
        ...
        if cnt % 10 == 0:
            ...
            with open(f'output/CNN2/plaintext.txt', 'a') as f:
                f.write(f'{accuracy:.2f}\n')
        cnt += 1
with open(f'output/CNN2/highest_plaintext.txt', 'a') as f:
